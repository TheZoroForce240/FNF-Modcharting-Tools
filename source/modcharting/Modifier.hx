...
class TipsyXModifier extends Modifier {
	...
	override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int) {
		noteData.x += currentValue * (FlxMath.fastCos(Conductor.songPosition * 0.001 * (1.2) + (lane % NoteMovement.keyCount) * (2.0)
			+ subValues.get('speed').value * (0.2)) * Note.swagWidth * 0.4);
	}
...
class TipsyYModifier extends Modifier {
	...
	override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int) {
		noteData.y += currentValue * (FlxMath.fastCos(Conductor.songPosition * 0.001 * (1.2) + (lane % NoteMovement.keyCount) * (2.0)
			+ subValues.get('speed').value * (0.2)) * Note.swagWidth * 0.4);
	}
...
class TipsyZModifier extends Modifier {
	...
	override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int) {
		noteData.z += currentValue * (FlxMath.fastCos(Conductor.songPosition * 0.001 * (1.2) + (lane % NoteMovement.keyCount) * (2.0)
			+ subValues.get('speed').value * (0.2)) * Note.swagWidth * 0.4);
	}
